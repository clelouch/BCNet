...
class Res2Net50(nn.Module):
    def __init__(self, baseWidth=26, scale=4):
        ...
        self.layer4 = self._make_layer(Bottle2neck, 512, 3, stride=1, dilation=2)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion or dilation == 2:
            downsample = nn.Sequential(
                nn.AvgPool2d(kernel_size=stride, stride=stride,
                             ceil_mode=True, count_include_pad=False),
                nn.Conv2d(self.inplanes, planes * block.expansion,
...
if __name__ == '__main__':
    images = torch.rand(2, 3, 224, 224).cuda()
    model = Res2Net50()
