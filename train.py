...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        ...
        for i, (images, gts, gt_edges) in enumerate(train_loader, start=1):
            ...
            [pred0, pred1, pred2, pred3], [edge0, edge1, edge2, edge3] = model(images)
            mask_loss0 = criterior_mask(pred0, gts)
            mask_loss1 = criterior_mask(pred1, gts)
            mask_loss2 = criterior_mask(pred2, gts)
            mask_loss3 = criterior_mask(pred3, gts)

            edge_loss0 = criterior_edge(edge0, gt_edges)
            edge_loss1 = criterior_edge(edge1, gt_edges)
            edge_loss2 = criterior_edge(edge2, gt_edges)
            edge_loss3 = criterior_edge(edge3, gt_edges)

            mask_loss = mask_loss0 + mask_loss1 + (mask_loss2 + mask_loss3) / 2
            edge_loss = edge_loss0 + edge_loss1 + (edge_loss2 + edge_loss3) / 2
            ...
            if i % 100 == 0:
                ...
                print(
                    '{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], eta: {}, MaskLoss0: {:0.4f}'.
                        format(datetime.now(), epoch, opt.epoch, i, total_step, eta_str, mask_loss0.data))
                logging.info(
                    '#TRAIN#: {} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], eta: {}, MaskLoss0: {:0.4f}, '
                    'EdgeLoss0: {:0.4f}'.
                        format(datetime.now(), epoch, opt.epoch, i, total_step, eta_str,
                               mask_loss0.data, edge_loss0.data))
        ...
        if (epoch) % 10 == 0:
            torch.save(model.state_dict(), os.path.join(save_path, 'models', 'BCNet_epoch_{}.pth'.format(epoch)))
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), os.path.join(save_path, 'models', 'BCNet_epoch_{}.pth'.format(epoch + 1)))
    ...
    with torch.no_grad():
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), os.path.join(save_path, 'models', 'BCNet_epoch_best.pth'))
